<template>
  <div class="xtx-cart-page">
    <div class="container">
      ...
      <div class="cart">
        <CartNone
          v-if="$store.getters['cart/validList'].length === 0"
        ></CartNone>
        <table>
          <thead>
            <tr>
            ...
            >
              <td>
                <XtxCheckbox
                  ...
                  @change="$store.dispatch('cart/updatedCart', item)"
                />
              </td>
              <td>
                <div class="goods">
...
export default {
  ...
  setup() {
    ...
    // 修改规格
    const updateCartSku = (oldSkuId, newSkuId) => {
      store.dispatch("cart/updateCartSku", { oldSkuId, newSkuId });
    };
    ...
    const batchDeleteCart = (isInvalid) => {
      //  点击删除时有一个确认弹窗
      Confirm({
        text: `您确定从购物车删除${isInvalid ? "失效" : "选中"}的商品吗？`,
      })
        .then(() => {
          console.log("点击确认");
    ...
    const goCheckout = () => {
      // 1.判断是否选择有效商品
      if (!store.getters["cart/selectedTotal"])
