<template>
  <XtxDialog v-model:visible="visibleDialog" title="查看物流">
    <div class="order-logistics-goods" v-if="logisitcs">
      <a class="image" href="javascript:;"
        ><img :src="logistics.picture" alt="" />
        <p>{{ logisitcs.count }}件商品</p></a
      >
    ...
    <div class="order-logistics-list" v-if="logisitcs">
...
export default {
  ...
  setup() {
    ...
    // 物流信息对象
    const logisitcs = ref(null);
    // 打开对话框
    const open = (item) => {
      visibleDialog.value = true;
      //    查询物流信息
      logisticsOrder(item.id).then(({ result }) => {
        console.log("物流信息", result);
        logisitcs.value = result;
      });
    };
    return { visibleDialog, logisitcs };
  },
};
</script>
