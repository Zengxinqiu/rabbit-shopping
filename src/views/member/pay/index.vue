<template>
  <div class="xtx-pay-page" v-if="findOrder">
    <div class="container">
      <XtxBread>
    ...
    <XtxDialog title="正在支付..." v-model="visibleDialog">
      <div class="pay-wait">
        ...
        <div v-if="findOrder">
          <p>如果支付成功：</p>
          <RouterLink :to="`/member/order/${findOrder.id}`"
            >查看订单详情></RouterLink
          >
          <p>如果支付失败：</p>
...
export default {
  ...
  setup() {
    ...
    findOrderDetail(route.query.id).then(({ result }) => {
      findOrder.value = result;
      console.log(result);
    ...
    return { findOrder, timeText, payUrl, visibleDialog };
  },
};
</script>
